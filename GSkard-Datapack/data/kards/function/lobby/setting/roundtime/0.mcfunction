...
tellraw @a [{translate: "kards.function.lobby.setting.choupai.11and15.1.1", fallback: "提示:",color: "gold"},{translate: "kards.function.lobby.setting.roundtime.0.1", fallback: "回合时间修改为",color: "green"},{translate: "kards.function.lobby.setting.roundtime.0.2", fallback: " 没有回合时间!直接进入PVP回合!抽取10张装备牌!抽取1件王之宝库装备",color: "red"}]
execute as @a at @s run playsound minecraft:block.amethyst_block.hit player @s ~ ~ ~ 100 2
setblock 99 228 -23 air
setblock 99 228 -23 minecraft:oak_wall_sign[facing=north]{front_text:{messages:[{click_event:{action:"run_command",command:"/function kards:lobby/setting/roundtime/30"},color:"gold",translate: "kards.function.lobby.setting.roundtime.0.3", fallback: "回合时间调整"},{translate: "kards.function.lobby.setting.roundtime.0.4", fallback: "没有回合时间!",color:"red"},'','']}} replace