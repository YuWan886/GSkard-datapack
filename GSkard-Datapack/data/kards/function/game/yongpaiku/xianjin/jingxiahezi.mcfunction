execute if entity @s[team=red] if score @s pingbi matches 0 run tellraw @a[team=red] [{"selector":"@s"},{"text":"使用了","color":"gold"},{"text":"[惊吓盒子]","color":"dark_aqua","hoverEvent":{"action":"show_text","value":"条件：敌方任意玩家使用法术牌时\n触发：在敌方场地召唤加速爆炸的苦力怕"}}]
...
execute if entity @s[team=blue] if score @s pingbi matches 0 run tellraw @a[team=blue] [{"selector":"@s"},{"text":"使用了","color":"gold"},{"text":"[惊吓盒子]","color":"dark_aqua","hoverEvent":{"action":"show_text","value":"条件：敌方任意玩家使用法术牌时\n触发：在敌方场地召唤加速爆炸的苦力怕"}}]
