execute if entity @s[team=red] if score @s pingbi matches 0 run tellraw @a[team=red] [{selector:"@s"},{translate: "game.paiku.fashu.wuyongfa.1",color:"gold"},{translate: "game.yongpaiku.xianjin.toulianghuanzhu.1",color:"dark_aqua",hover_event:{action:"show_text","value":"条件：敌方回合结束时\n触发：记录敌方一名K点最多的玩家的K点 在下回合时使其均分给我方所有玩家 每名玩家最多得到6点(不取余数)"}}]
...
execute if entity @s[team=blue] if score @s pingbi matches 0 run tellraw @a[team=blue] [{selector:"@s"},{translate: "game.paiku.fashu.wuyongfa.1",color:"gold"},{translate: "game.yongpaiku.xianjin.toulianghuanzhu.1",color:"dark_aqua",hover_event:{action:"show_text","value":"条件：敌方回合结束时\n触发：记录敌方一名K点最多的玩家的K点 在下回合时使其均分给我方所有玩家 每名玩家最多得到6点(不取余数)"}}]
