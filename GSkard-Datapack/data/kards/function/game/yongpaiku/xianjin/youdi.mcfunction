execute if entity @s[team=red] if score @s pingbi matches 0 run tellraw @a[team=red] [{"selector":"@s"},{"text":"使用了","color":"gold"},{"text":"[诱敌]","color":"dark_aqua","hoverEvent":{"action":"show_text","value":"条件：敌方任意玩家回合内使用第2张法术牌时\n触发：该法术失效"}}]
...
execute if entity @s[team=blue] if score @s pingbi matches 0 run tellraw @a[team=blue] [{"selector":"@s"},{"text":"使用了","color":"gold"},{"text":"[诱敌]","color":"dark_aqua","hoverEvent":{"action":"show_text","value":"条件：敌方任意玩家回合内使用第2张法术牌时\n触发：该法术失效"}}]
