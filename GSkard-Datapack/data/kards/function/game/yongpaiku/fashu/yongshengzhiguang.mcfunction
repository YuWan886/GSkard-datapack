execute if score @s pingbi matches 0 run tellraw @a [{selector:"@s"},{translate: "game.yongpaiku.template.1",color:"gold"},{translate: "game.yongpaiku.fashu.xinlingfenshao.1",color:"dark_purple",hover_event:{action:"show_text","value":"使敌方队伍所有生物凋零（包括图腾与怪物）"}}]
item replace entity @s weapon.offhand with air
scoreboard players operation @s kardCount -= #kard_yongshengzhiguang kardCount
