...
execute if entity @s[team=blue] as @e[distance=..3,team=red,tag=!spectator,limit=3,sort=random] run damage @s 5 magic by @n[tag=kunanhuashen_damage]

execute on owner run tag @s remove kunanhuashen_damage
execute summon marker run function kards:game/yongpaiku/shenji/wangzhibaoku/kunanyaolan/particle/1
