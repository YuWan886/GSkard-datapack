execute if score @s pingbi matches 0 run tellraw @a [{selector:"@s"},{text: "使用了",color:"gold"},{text: "[灾厄双星]",color:"dark_green",hover_event:{action:"show_text","value":"在敌方召唤一只幻术师与唤魔者"}}]
   #红队用
    ...
    #蓝队用
    execute if entity @s[team=blue] as @e[tag=6b] at @s run summon minecraft:illusioner ~ 0 ~ {Team:blue,attributes:[{id:"minecraft:max_health",base:30.0d}],Health:30.0f,equipment:{mainhand:{id:"minecraft:bow"}},}
