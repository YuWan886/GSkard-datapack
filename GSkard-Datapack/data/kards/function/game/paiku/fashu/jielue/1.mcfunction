execute if score @s pingbi matches 0 run tellraw @a [{selector:"@s"},{translate: "game.paiku.fashu.wuyongfa.1",color:"gold"},{translate: "game.paiku.fashu.jielue.1.1",color:"dark_purple",hover_event:{action:"show_text","value":"掠夺一只敌方图腾"}}]
...
#红
execute if entity @s[team=red] unless entity @e[tag=tuteng,team=blue] run tellraw @a [{translate: "game.paiku.fashu.jielue.1.2",color:"gray"}]
...
#蓝
execute if entity @s[team=blue] if entity @s[team=blue] if score 红队 xianjin_shufashixiao matches 0 unless entity @e[tag=tuteng,team=red] run tellraw @a [{translate: "game.paiku.fashu.jielue.1.3",color:"gray"}]
